...
def test_plan_safety_block_bulk(mock_config, mock_logger):
    ...
    plan = RequestPlan(
        ...
        tasks=[
            PlannedTask(id="1", service="drive", action="delete_file", parameters={"file_id": "123"})
        ]
    )
...
def test_plan_safety_block_too_many_destructive(mock_config, mock_logger):
    ...
    tasks = [
        PlannedTask(id=str(i), service="drive", action="delete_file", parameters={"file_id": str(i)})
        for i in range(5)
    ]
