...
from .config import OPENROUTER_DEFAULT_BASE_URL, OPENROUTER_DEFAULT_MODEL
...
def discover_gws_binary() -> Path | None:
    ...
    configured = os.getenv("GWS_BINARY_PATH", "").strip()
    candidates = [
        Path.cwd() / "gws",
    ]
    if configured:
    ...
    for executable in tuple(item for item in ("gws", configured, "google-workspace-cli") if item):
...
def _render_env(values: dict[str, str]) -> str:
    lines = [
        ...
        "# Only OpenRouter free models are supported.",
