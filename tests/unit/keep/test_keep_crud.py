...
class TestKeepCRUD:
    ...
    def test_keep_lifecycle_execution(self, executor, runner):
        ...
        runner.run.side_effect = [
            # Create call
            ExecutionResult(success=True, command=[], stdout=json.dumps({"name": "notes/created123", "title": "Life"})),
            # Triple-check calls (3 times get_note)
            ExecutionResult(success=True, command=[], stdout=json.dumps({"name": "notes/created123"})),
            ExecutionResult(success=True, command=[], stdout=json.dumps({"name": "notes/created123"})),
            ExecutionResult(success=True, command=[], stdout=json.dumps({"name": "notes/created123"})),
            # List call
            ExecutionResult(success=True, command=[], stdout=json.dumps({"notes": [{"name": "notes/created123"}]})),
            # Delete call
        ...
        # 3. Delete Note Task
        delete_task = PlannedTask(id="t3", service="keep", action="delete_note", parameters={"name": "notes/created123"})
    ...
    def test_create_note_triple_check_fail(self, mock_sleep, executor, runner):
        ...
        runner.run.side_effect = [
            # Create success
            ExecutionResult(success=True, command=[], stdout=json.dumps({"name": "notes/fail", "title": "F"})),
