...
def test_config_provider_specific_model_fallback_for_openai(monkeypatch):
    monkeypatch.setenv("LLM_PROVIDER", "openai")
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.setenv("OPENAI_MODEL", "gpt-openai-fallback")
    config = AppConfig.from_env()
    assert config.model == "gpt-openai-fallback"
