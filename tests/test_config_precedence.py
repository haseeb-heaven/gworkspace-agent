                    ...
                    with patch('gws_assistant.langgraph_workflow.run_workflow', side_effect=KeyboardInterrupt):
                        with patch('gws_assistant.config.AppConfig.from_env') as mock_from_env:
                            ...
                            base_config = AppConfigModel(
                                provider="openrouter", model="m", api_key="k", base_url="b",
                                timeout_seconds=30, gws_binary_path=Path("gws"),
                                log_file_path=Path("l"), log_level="INFO", verbose=False,
                                env_file_path=Path(".env"), setup_complete=True, max_retries=3,
                                langchain_enabled=True, sandbox_enabled=True, read_only_mode=True
                            )
